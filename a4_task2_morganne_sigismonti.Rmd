...
### 3. Preliminary Results - Annual juvenile hare trap counts
...
```
Juvenile Snowshoe Hare trap counts per year are shown in Figure 1. The minimum and maximum number of juvenile hares counted was 2 hares in 2010 and 126 hares in 1999. The mean and median annual hare trap counts were `r bonanza_ann_juv_stats$Mean` and `r bonanza_ann_juv_stats$Median`. From year 1999 to 2012, there is an overall decrease in the amount of juvenile snowshoe hare samples collected per year. Moving forward, the data collectors should standardize their data collection to minimize confounding variables in their data. For example, they can try to collect juvenile hares at similar times of the year. 
...
**Table 1.** Descriptive statistics (mean, standard deviation, and sample size) for weights of female and male juvenile snowshoe hares.
...
# Overall: My assumption that the data is normally distributed holds true. Sample size is normally and therefore Central Limit Theorem (means will be normally distributed). This means that I can use the Two-sample t-test to compare my data!
